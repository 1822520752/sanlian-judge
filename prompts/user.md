...
输出字段:
- `name`: 匹配的 UP 主名称(必须真实存在的 B 站 UP 主)
- `mid`: 匹配的 UID(字符串)
- `similarity`: 相似度百分比(整数 0-100)
- `reason`: 匹配理由(100 字左右)

规则:
- MVP 阶段:从知名 B 站 UP 主池中挑一位精神最匹配的
- 同一认证类型 / 调性的 UP 主 → 高相似度
- 粉丝量级接近 → 高相似度
- 最终结果要有说服力 + 有趣
- 候选池参考(可直接使用): 老番茄(546195)、何同学(46143887)、罗翔说刑法(517327498)、影视飓风(946974)、硬核的半佛仙人(390461123)、盗月社食遇记(394024400)、宝剑嫂(3493145548601811)、中国boy超级大猩猩(16696694)、绵羊料理(2981396)、敬汉卿
